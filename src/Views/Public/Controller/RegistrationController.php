...
class RegistrationController extends AbstractController
{
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $this->emailVerifier->sendEmailConfirmation('app_verify_email', $user,
                (new TemplatedEmail())
                    ...
                    ->to($user->getEmail())
                    ->subject('Потверждение Email адреса')
            ...
            $this->addFlash('success', 'Вы успешно зарегистрировались, ссылка подтверждения отправлена на Email адрес');
        }
    ...
    public function verifyUserEmail(Request $request, TranslatorInterface $translator): Response
    {
        if (false===$this->isGranted('IS_AUTHENTICATED_FULLY')) {
            $this->addFlash('info', 'Выполните вход');
        }
